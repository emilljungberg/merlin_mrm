...
def main():
    ...
    moco_img = h5py.File('moving_phantom_moco-cg.h5', 'r')['image'][0, ...]
    ...
    view_slice = np.s_[50, :, :]
    static_slice = np.rot90(abs(static_img[view_slice]), 2)
    ...
    moving_slice = np.rot90(abs(moving_img[view_slice]), 2)
    ...
    moco_slice = np.rot90(abs(moco_img[view_slice]), 2)
